...
export default async function PortalDashboard() {
  ...
  try {
    ...
    console.error("PortalDashboard Error:", error);
    return (
      <div className="p-20 text-center">
        <h1 className="text-2xl font-bold text-red-600">Failed to load portal</h1>
        <p className="text-gray-600 mt-2">Please try again later. If the issue persists, contact support.</p>
        <pre className="mt-4 p-4 bg-gray-100 rounded text-left overflow-auto max-w-2xl mx-auto text-xs">
          {error.message}
        </pre>
      </div>
    );
