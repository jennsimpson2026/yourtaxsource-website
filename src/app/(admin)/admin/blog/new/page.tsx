...
import { createPost, getCategories } from "@/actions/admin/blog";
import { 
  ArrowLeft, 
  Save, 
  Eye, 
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h1 className="text-3xl font-heading font-bold text-brand-black">Create New Post</h1>
        </div>
          ...
          <div className="bg-brand-navy p-6 rounded-3xl shadow-sm text-white space-y-4">
            ...
            <ul className="text-xs space-y-3 text-white/60">
              <li className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${formData.title.length > 10 ? 'bg-green-500' : 'bg-white/20'}`}></div>
                Catchy title (min 10 chars)
              </li>
              <li className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${formData.content.length > 100 ? 'bg-green-500' : 'bg-white/20'}`}></div>
                Substantial content
              </li>
              <li className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${formData.featuredImageUrl ? 'bg-green-500' : 'bg-white/20'}`}></div>
                Featured image set
              </li>
            </ul>
          </div>
