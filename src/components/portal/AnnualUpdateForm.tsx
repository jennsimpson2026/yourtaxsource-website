    ...
    <div className="bg-white rounded-[2rem] shadow-xl border border-gray-100 overflow-hidden max-w-4xl mx-auto">
      ...
      <div className="p-8 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
        <button 
          onClick={prevStep}
          disabled={step === 1}
          className={`flex items-center gap-2 font-bold text-sm transition-all ${step === 1 ? 'text-gray-300 cursor-not-allowed' : 'text-brand-charcoal hover:text-brand-black'}`}
        >
        ...
        {step < 6 ? (
          ...
          <button 
            onClick={() => alert("Form submitted! In a real app, this would call an API.")}
            className="bg-brand-black text-white px-8 py-3 rounded-xl font-bold text-sm hover:bg-opacity-90 transition-all flex items-center gap-2 shadow-lg shadow-brand-black/20"
          >
...
function TimelineStep({ number, label, status, icon }: any) {
  return (
    <div className="flex flex-col items-center gap-2 relative z-10">
      <div className={`
        w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300
        ${status === 'current' ? 'bg-brand-purple border-brand-purple text-white scale-110 shadow-lg shadow-brand-purple/20' : 
          status === 'completed' ? 'bg-green-500 border-green-500 text-white' : 
          'bg-brand-black border-gray-700 text-gray-500'}
      `}>
        {status === 'completed' ? <CheckCircle2 size={20} /> : icon}
      </div>
      <span className={`text-[10px] font-bold uppercase tracking-widest hidden md:block ${status === 'current' ? 'text-brand-purple' : 'text-gray-500'}`}>
        {label}
      </span>
    </div>
  );
