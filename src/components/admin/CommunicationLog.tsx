import { Clock, Send, FileUp, Mail } from "lucide-react";
...
export function CommunicationLog({ logs }: { logs: AuditLog[] }) {
  ...
  const commActions = ["REQUEST_DOCUMENTS", "UPDATE_RETURN_STATUS"];
            ...
            <div key={log.id} className="p-5 hover:bg-brand-cloud/30 transition-colors">
              <div className="flex items-start gap-4">
                <div className={`mt-1 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                  log.action === 'REQUEST_DOCUMENTS' ? 'bg-orange-50 text-brand-orange' : 'bg-blue-50 text-brand-navy'
                }`}>
                  {log.action === 'REQUEST_DOCUMENTS' ? <Send size={14} /> : <FileUp size={14} />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <p className="text-sm font-bold text-brand-navy">
                      {log.action === 'REQUEST_DOCUMENTS' ? 'Document Request Sent' : 'Status Update Sent'}
                    </p>
