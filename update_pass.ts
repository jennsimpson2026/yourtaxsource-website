...
async function main() {
  const newPassword = await bcrypt.hash("StagingAdmin123!", 10);
  console.log("Updating password for jsimpson@yourtaxsource.com...");
  const result = await client.execute({
    sql: "UPDATE users SET password = ? WHERE email = ?",
    args: [newPassword, "jsimpson@yourtaxsource.com"],
  });
